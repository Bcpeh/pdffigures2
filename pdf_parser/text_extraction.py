...
def extract_sections(client: OpenAI, text, section_mapping_path=SECTION_MAPPING):
    ...
    if (len(structured_data) < 5) or (len(structured_data) > 12):
        structured_data = extract_sections_with_llm(client, text, section_mapping_path)
    section_positions = get_section_positions(structured_data)
    if structured_data.get("references", ""):
        structured_data = update_structured_data_with_appendices(client, structured_data)
...
def mapping_sections_with_llm(client: OpenAI, structured_data: dict, section_mapping_path: str = SECTION_MAPPING):
    ...
    try:
        ...
        result = messages.choices[0].message.content
...
def extract_sections_with_llm(client: OpenAI, text: str, section_mapping_path: str = SECTION_MAPPING):
...
def update_structured_data_with_appendices(client: OpenAI, structured_data: dict):
